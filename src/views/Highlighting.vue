...
<script>
	...
	export default {
		...
		setup () {
			...
			const highlight = ref('');
			const inputArray = computed(() => { return input.value.split('');
            });
            const inputArrayLength = computed(() => { return inputArray.value.length + 1;});

            const highlightedWord = computed(() => {
                var highlightSplit = highlight.value.split('');
